...
- 几个概念区分：响应时间、性价比、吞吐率
- Amdahl 定律
	- 假设原来在一个系统中执行一个程序需要时间 $T_{old}$ ，其中某一个部分占的时间百分比为 $\alpha$ ，然后，把这一部分的性能提升 $k$ 倍。即这一部分原来需要的时间为 $\alpha T_{old}$ ，现在需要的时间变为 $(\alpha T_{old})/k$ 。则整个系统执行此程序需要的时间变为：$T_{new} = (1-\alpha)T_{old}+(\alpha T_{old})/k$
	- 故可得，系统性能提速的倍数为: $S=\frac{T_{old}}{T_{new}}=\frac{1}{(1-\alpha)+\alpha/k}$
