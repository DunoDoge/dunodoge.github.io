...
- 数据校验
	...
	- 奇偶校验码
		- 码距 d=2
		- 只能发现奇数位错误（一字节通常错一位）
	- ❗ 海明码 Hamming code ![教材表 6.1.png](<img/cod-table-6-1.png>)
...
- 6 种指令格式 R | I | S | B | U | J ![教材图 4.6.png](<img/cod-figure-4-6.png>)
- 过程调用约定
	...
	- 用户栈的状态变化示例 ![教材图 4.13.png](<img/cod-figure-4-13.png>)
		...
		- 数据冒险 data hazards - 后面的指令要用前面指令的结构
			- 写后读数据冒险 read after write, RAW
				...
				- 特例：Load-use 数据冒险
					...
					- 只能编译优化 / 插入 nop
		- 控制冒险 control hazards - 由分支指令引起的流水线阻塞 ![教材图 5.57.png](<img/cod-figure-5-57.png>)
			- 延迟损失时间片 C
			- 简单预测（总是不转移）/ 动态预测
		- 流水线周期图分析
...
- 高速缓存 cache
	...
	- 信息交换单位——主存块 / cache 行
		- cache 有效位，确认信息是否有效
		- cache 对程序员是透明的 ![教材图 6.24.png](<img/cod-figure-6-24.png>)
	- 块映射到 cache 行
		- **直接映射** 主存块与 cache 行“多对一”严格对应
			- cache 行号 = 主存块号 mod cache 行数 ![教材图 6.25.png](<img/cod-figure-6-25.png>)
...
- 虚拟地址空间
	...
	- 全相联映射，以提高命中率（缺页处理慢）
	- 回写法（外存访问慢）
	- **页表**描述虚拟页对应的主存页框号 / 磁盘存储位置 ![教材图 6.40.png](<img/cod-figure-6-40.png>)
	- 快表 TLB
		- 保存在高速缓存中
		- TLB 标记字段 + 页表项内容（tag 原理同 cache）
		- TLB 满，通常采用随机替换
	- 一个典型的 CPU 访存过程 ![教材图 6.44.png](<img/cod-figure-6-44.png>)
